...
file_path <- "C:/Users/KASHISH NIGAM/Desktop/Last Sem Project/Final Datasets/USA/uniform_distributions-During_Covid_USA.xlsx"

# Load the data from the Excel file
During_covid_USA <- read_excel(file_path)

# Convert the data to a matrix (if not already in matrix form)
u_data <- as.matrix(During_covid_USA)
...
alpha <- 2.189  # Replace with your Plackett copula parameter estimate
...
pearson_rho <- cor(During_covid_USA, method = "pearson")

# Compute Kendall's tau
kendall_tau <- cor(During_covid_USA, method = "kendall")
