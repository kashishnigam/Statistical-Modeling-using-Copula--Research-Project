...
file_path <- "C:/Users/KASHISH NIGAM/Desktop/Last Sem Project/Final Datasets/USA/uniform_distributions-Pre_Covid_USA.xlsx"

# Load the data from the Excel file
pre_covid_USA <- read_excel(file_path)

# Convert the data to a matrix (if not already in matrix form)
u_data <- as.matrix(pre_covid_USA)
...
alpha <- 1.879  # Replace with your Plackett copula parameter estimate
...
pearson_rho <- cor(pre_covid_USA, method = "pearson")

# Compute Kendall's tau
kendall_tau <- cor(pre_covid_USA, method = "kendall")
